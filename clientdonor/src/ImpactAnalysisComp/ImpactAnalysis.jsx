  ...
  const fetchAnalysis = async () => {
    ...
    try {
      ...
      const updatedYearlyTrends = response.data.yearly_trends.map((trend) => {
        ...
        if (trend.Year === 2020) fluctuation = 0.1;
      ...
      <div style={{ padding: "20px" }}>
            ...
            <PieChart width={800} height={400}>
              ...
              >
                {pieData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={["#8884d8", "#82ca9d", "#ff8c00", "#ff0000"][index]}
                  />
                ))}
              </Pie>
              <PieTooltip />
        ...
        {Object.keys(inferences).length > 0 && (
          ...
          >
            ...
            <ul>
              <li>
                Landfills Increase: {inferences["Landfills Increase (%)"]}%
              </li>
              <li>Fire Risk: {inferences["Fire Risk (%)"]}%</li>
              <li>
                Composting Needs Increase:{" "}
                {inferences["Composting Needs Increase (%)"]}%
              </li>
            </ul>
          </motion.div>
        )}
